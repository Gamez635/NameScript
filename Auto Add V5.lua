...
local function formatCFrame(cf)
    ...
    return string.format("CFrame.new(%.3f,%.3f,%.3f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f)",x or 0,y or 0,z or 0,r00 or 1,r01 or 0,r02 or 0,r10 or 0,r11 or 0,r12 or 1,r20 or 0,r21 or 0,r22 or 0)
end
...
createBtn("Export Seg",Color3.fromRGB(0,180,220),function()
    ...
    for seg=1,total do
        ...
        local code="-- V23 Seg "..seg.."\nlocal RunService=game:GetService(\"RunService\")\nlocal lp=game.Players.LocalPlayer\nlocal cam=workspace.CurrentCamera\nlocal char=lp.Character or lp.CharacterAdded:Wait()\nlocal hrp=char:WaitForChild(\"HumanoidRootPart\")\nlocal hum=char:WaitForChild(\"Humanoid\")\nhum.AutoRotate=false\ncam.CameraType=Enum.CameraType.Scriptable\nlocal frames={"
        for i,f in ipairs(sf) do
            local rel=f.Time-sf[1].Time
            code=code..string.format("{Pos=Vector3.new(%.3f,%.3f,%.3f),Vel=Vector3.new(%.3f,%.3f,%.3f),Move=Vector3.new(%.3f,0,%.3f),Jump=%s,CFrame=%s,FOV=%.2f,Time=%.4f}%s",
                f.Pos.X,f.Pos.Y,f.Pos.Z,f.Vel.X,f.Vel.Y,f.Vel.Z,f.Move.X,f.Move.Z,tostring(f.Jump),formatCFrame(f.CFrame),f.FOV or 70,rel,i<#sf and "," or "")
        end
        ...
        if writefile then writefile("V23_Seg_"..string.format("%03d",seg)..".lua",code) end
    end
end)
...
createBtn("Full Loop",Color3.fromRGB(220,100,0),function()
    ...
    for i,f in ipairs(frames) do
        ...
        code=code..string.format("{Pos=Vector3.new(%.3f,%.3f,%.3f),Vel=Vector3.new(%.3f,%.3f,%.3f),Move=Vector3.new(%.3f,0,%.3f),Jump=%s,CFrame=%s,FOV=%.2f,Time=%.4f}%s",
            f.Pos.X,f.Pos.Y,f.Pos.Z,f.Vel.X,f.Vel.Y,f.Vel.Z,f.Move.X,f.Move.Z,tostring(f.Jump),formatCFrame(f.CFrame),f.FOV or 70,rel,i<#frames and "," or "")
    end
    ...
    if writefile then writefile("V23_Full_Loop.lua",code) end
end)
...
createBtn("Non-Loop",Color3.fromRGB(0,200,170),function()
    ...
    if writefile then writefile("V23_NonLoop.lua",code) end
end)

createBtn("Play",Color3.fromRGB(60,200,60),function(btn)
    if isPlaying then isPlaying=false btn.Text="Play" return end
    if #frames<5 then return end
    isPlaying=true btn.Text="STOP"
    local char=lp.Character or lp.CharacterAdded:Wait()
    local hum=char:WaitForChild("Humanoid")
    local hrp=char:WaitForChild("HumanoidRootPart")
    local cam=workspace.CurrentCamera
    local oc=cam.CameraType local orot=hum.AutoRotate
    hum.AutoRotate=false cam.CameraType=Enum.CameraType.Scriptable
    local st=tick()
    local c; c=RunService.RenderStepped:Connect(function()
        if not isPlaying then c:Disconnect() cam.CameraType=oc hum.AutoRotate=orot btn.Text="Play" return end
        local el=tick()-st
        if el>frames[#frames].Time then isPlaying=false c:Disconnect() cam.CameraType=oc hum.AutoRotate=orot btn.Text="Play" return end
        local idx=1 while idx<#frames and el>=frames[idx+1].Time do idx+=1 end
        if idx>=#frames then return end
        local a=frames[idx] local b=frames[idx+1]
        ...
        if flat.Magnitude>0 then hrp.CFrame=CFrame.new(a.Pos:Lerp(b.Pos,t),a.Pos:Lerp(b.Pos,t)+flat*10) end
        hum:Move(a.Move:Lerp(b.Move,t),true)
        hrp.AssemblyLinearVelocity=a.Vel:Lerp(b.Vel,t)
        if (t<0.5 and a.Jump)or(t>=0.5 and b.Jump)then hum.Jump=true end
    end)
end)
...
RunService.Heartbeat:Connect(function()
    ...
    local now=tick() local dt=now-lt if dt<0.013 then return end lt=now
    table.insert(frames,{
        Pos=root.Position,
